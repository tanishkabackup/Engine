...
export default function UpdateTaskForm({ task, memberId, onBack }: UpdateTaskFormProps) {
  ...
  const { register, handleSubmit, reset, formState: { isSubmitting } } = useForm<DailyTaskUpdateFields>({
    defaultValues: {
      status: STATUS_OPTIONS[0],
      updatedEta:new Date(), 
      comment: "",
      effortHours: task.hours,
    }
  });
  ...
  const onSubmit =(data: DailyTaskUpdateFields) =>{
    const request: DailyTaskUpdateRequest={
      taskId: task.taskId,
      status: data.status,
      updatedEta: new Date(data.updatedEta),
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <select 
            ...
            className="bg-sky-50 text-sky-700 border-none rounded-full px-4 py-2 font-black text-xs uppercase tracking-wider focus:ring-2 focus:ring-sky-500 outline-none cursor-pointer"
          >
